...
description: Communication patterns, framing techniques, and collaboration rhythms for getting the best results from Bonsai-powered agents.
---
...
Each phase can also be triggered via slash command for workflows that map to it — `/planning` for COMMIT, `/security-audit` or `/code-review` for VERIFY, `/pr-review` before SHIP. See [Triggers and Activation](/Bonsai/guides/triggers-and-activation/) for the full list.
...
<Aside type="tip">
Make verification a **phase**, not an afterthought. Don't rush from implementation to shipping. The verification phase regularly takes 10-20% of the session and catches issues that would cost 10x more to fix after shipping.
</Aside>
