...
Keep `station/Playbook/Status.md` clean — archive old Done items, validate Pending items, verify plan files match Status rows.
...
3. **Verify plan files match Status rows:**
   - Scan `station/Playbook/Plans/Active/` — each file should correspond to an In Progress or Recently Done row in Status.md (by plan number)
   - Flag any orphaned plan files (no matching Status row)
   - Flag any Status row that references a plan number with no matching file in `Plans/Active/` or `Plans/Archive/`
