...
**Current task:** Plan 22 — `bonsai init` cinematic redesign. Phases 1–3 shipped 2026-04-21 (PRs #47 `7553d43`, #48 `2e2a08c`, #49 `971ee44`) + Phase 3.5 direct-to-main dogfood polish pass 2026-04-21. Next: Phase 4 (Branches tabbed picker + inline-expand — replaces Branches stub with real tabbed category picker per `zen-shell.jsx` ZStepBranches).
**In flight (other tracks):** None.
**Blocked on:** Nothing.
**Loose ends (uncommitted):** Phase 3.5 polish edits across 7 files (styles, chrome, enso, soil, stage, vessel, memory) + Status update — pending one wrap commit.
**Last completed:** PR #49 `971ee44` — Plan 22 Phase 3: `VesselStage` (vessel.go 278L, vessel_test.go 185L) with 3× `bubbles/textinput` + tab/shift-tab focus cycling + required validation (Name + Station) + `Result() map[string]string` + station default `station/` on empty; `SoilStage` (soil.go 261L, soil_test.go 179L) hand-rolled multi-select (NOT `bubbles/list`) + `◆`/`◇` glyphs + required-pinned + REQUIRED badge (Bark) + Leaf left-border focus + `Result() []string` in input order; `RenderHeader` signature dropped `stationSubdir` (user-reported bug: `station/` segment in header was misleading since subdir doesn't exist pre-generate) → renders project path only (`~/.../project/`); `Stage.renderFrame` call site updated; `Stage.stationDir` + `StageContext.StationDir` retained (Phase 5 Planted body will use); `cmd/init_redesign.go` wires `NewVesselStage(ctx)` + `NewSoilStage(ctx, options)` in place of stubs, Branches + Observe still stubs; chrome_test.go (65L, 4 tests) guards no-station-in-header regression. Net 1019 adds / 25 dels across 8 files. CI green (test/Analyze Go/lint/govulncheck/CodeQL/GitGuardian). Independent review PASS on all 8 checks. Authorized divergence: validator logic inlined in Vessel (can't import `cmd` from `initflow` — cycle); empty station → default `station/` instead of rejecting (plan-authorized). Pre-dispatch refinements: Vessel `Result()` shape resolved → `map[string]string{name,description,station}`; Soil implementation resolved → hand-roll (4–8 items; `bubbles/list` overkill); header strip bundled. Prior: PR #48 `2e2a08c` — Plan 22 Phase 2: `internal/tui/initflow/` package (chrome.go 157L, enso.go 261L, fallback.go 93L + test 65L, stage.go 189L, stub.go 77L) + `harness.Chromeless` capability + test + env-flag branch in `cmd/init.go` + stub orchestrator `cmd/init_redesign.go` + runewidth promoted indirect→direct. Net 1029 adds / 1 del. Legacy `bonsai init` (no env var) unchanged. Prior: PR #47 `7553d43` — Plan 22 Phase 1: `tui.RenderFileTree` widget + `ColorLeafDim`/`ColorRule`/`ColorRule2` palette tokens. Net 604 adds, 0 dels. Prior: PR #46 `d14edbe` — Plan 21 session-start context dedup + Plan 08 Phase C sensors.
