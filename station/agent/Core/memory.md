...
- **Local `go build`/`go test` miss `golangci-lint unused`.** Local golangci-lint v2.x; repo config is v1 → `golangci-lint run` errors locally with "unsupported version". Trust CI for dead-code on deletion/refactor PRs, OR install v1 locally (`go install github.com/golangci/golangci-lint/cmd/golangci-lint@v1.64.8`). *(stale — repo config migrated to v2 in Plan 20/PR #29; `.golangci.yml` now has `version: "2"` so local v2.x matches. The v1/v2 mismatch error is resolved; general CI-trust advice may still apply for other reasons.)*
