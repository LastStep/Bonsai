...
- **`isolation:"worktree"` agents leak edits to the MAIN checkout.** Observed repeatedly 2026-06-13 (Plan 40 dispatch): background worktree agents' `cd`-prefixed commands reset cwd to the main repo while their `Edit`/`Write` (absolute paths) landed in the main checkout, not the worktree — Phase 3's writes even clobbered Phase 2's in-flight WIP. Mitigation that worked: tell each agent its exact `$WT` absolute path, edit only under `$WT`, run all git via `git -C $WT`, and BEFORE committing assert `git -C <main> status` is empty + edits show in `$WT`. For fixes on an existing PR branch, dispatch a NON-isolation agent pointed at the existing worktree (isolation:worktree would fork a new branch off main). Also: `bonsai init --non-interactive` refuses an existing `.bonsai.yaml` by design (`nonint/runner.go:48`, exit 4) — no CLI path re-scaffolds an initialized repo until Phase-4 `bonsai update` delivery lands.
...
- **`syscall.O_NOFOLLOW` is POSIX-only — breaks Windows cross-compile.** Plan 32 (PR #80) added it to `internal/generate/catalog_snapshot.go:204`; Linux `go build` + `go test` + `govulncheck` all passed; only `goreleaser release` cross-compile to GOOS=windows surfaced `undefined: syscall.O_NOFOLLOW`. v0.3.0 predated the bug; v0.4.0 first release affected. *Why:* CI runs Linux-only build/test; cross-platform syscall use isn't gated by any local check. *How to apply:* (a) when adding any `syscall.*` constant, immediately verify `GOOS=windows GOARCH=amd64 go build ./...` locally; (b) use platform-split files (`_unix.go` `//go:build !windows` + `_windows.go` `//go:build windows`) for the helper rather than scatter build tags inline; (c) consider adding a Windows cross-compile step to `ci.yml` lint job to catch this class earlier. Hotfix path: PR #95 splits `openSnapshotFile` per platform, force-moved `v0.4.0` tag, used Plan 36's new `workflow_dispatch` retry hook to re-run release.
- **`inspect_swe` (Meridian Labs) is trusted-conditionally for Bonsai-Eval rungs 1+2.** Org = 501(c)(3) JJ Allaire (Inspect AI lead) with UK AISI + Apollo Research contributors; ~108k PyPI downloads/30d; CI w/ ruff+mypy+pytest. Caveats: pre-1.0 (0.2.51), bus factor 82% jjallaire, replacement cost ~500-1000 LOC. *How to apply:* always exact-pin (`inspect-swe==0.2.51`), commit `uv.lock`, fork to `LastStep/inspect_swe-frozen` as safety net, no upgrades during measurement windows. Never write our own rung-1/rung-2 wrapper from scratch — cost/benefit favors reuse. Re-evaluate if Meridian goes silent for 3+ months or if a v1.0 ships with breaking changes.
- **Public leaderboard numbers ≠ rung-1/rung-2 substitute.** Terminal-Bench/LiveCodeBench leaderboards omit temp/max_tokens/tool list/cost per entry. Anthropic's SWE-bench post describes scaffold but doesn't release configs. HAL enforces reproducibility but covers wrong benchmark suite (no Terminal-Bench/LiveCodeBench). *How to apply:* cite leaderboards as cross-reference / sanity-check only; always run our own rung-1/2 under pre-registration to defend "apples-to-apples" claim.
- **Max-plan OAuth doesn't cover SDK-direct calls.** `claude` CLI uses Max OAuth; `anthropic` Python SDK + Inspect AI's `--model anthropic/...` substrate calls + judge models all require `ANTHROPIC_API_KEY`. *How to apply:* any eval harness mixing rung-1 (raw API) + Inspect substrate needs API key regardless of Max plan. Personal billing per Plan 38 Locked Decisions.
- **Run `bonsai validate` on the dogfood repo after big merges.** Plan 35 caught two real bugs in our own repo immediately after ship: `bubbletea.md` + `statusline.sh` both lacked frontmatter (file dropped manually, never registered). `validate` is the cheapest signal that workspace state has drifted. *Why:* this is a project that ships the validate command — running it on ourselves is both correctness check and dogfood demo. *How to apply:* after any merge that touches `internal/generate/scan.go`, `internal/validate/`, or workspace structure, run `./bonsai validate`. Treat exit ≠ 0 as triage signal, not noise.
...
- **Consolidate disjoint phases into ONE PR — don't open one PR per phase.** Feedback 2026-06-16 (Plan 41): dispatching P2/P3/P4 as 3 parallel worktree agents produced 3 separate PRs/pushes + a 4th for P1 + 5th for P5 = too many PRs/pushes for one plan. **Why:** PR/push churn, not main-branch commit count (squash already collapses intra-phase commits to one per PR). **How to apply:** keep parallel agents for speed, but land their file-disjoint branches onto a single integration branch → ONE PR → one push (review the combined diff once, or per-area before integrating). Reserve a standalone PR only for a phase that MUST merge first due to a signature/type dependency (e.g. Plan 41 P1 changed `RunInit`/`RunAdd` sigs → unavoidable solo merge). Target: ≤2-3 PRs for a 5-phase plan, not 5. Push when a reviewable unit is done, not per intra-phase commit.
...
- **Foundational research docs** — Anchor for methodology/concept decisions. *(gitignored local-only files — not tracked in repo; validated absent from current environment 2026-08-04; may exist on user's primary machine)*
    - [Research/RESEARCH-landscape-analysis.md](../../Research/RESEARCH-landscape-analysis.md) — Bonsai vs GSD/ECC/others; identity/coordination layer positioning *(stale — file not found in current environment; gitignored local-only)*
    - [Research/RESEARCH-concept-decisions.md](../../Research/RESEARCH-concept-decisions.md) — Ambient vs command-driven, authority hierarchy, catalog ownership, talents taxonomy *(stale — file not found in current environment; gitignored local-only)*
    - [Research/RESEARCH-eval-system.md](../../Research/RESEARCH-eval-system.md) — Eval system concept: scenarios, evaluators, benchmarks *(stale — file not found in current environment; gitignored local-only)*
    - [Research/RESEARCH-trigger-system.md](../../Research/RESEARCH-trigger-system.md) — Trigger section design research *(stale — file not found in current environment; gitignored local-only)*
    - [Research/RESEARCH-uiux-overhaul.md](../../Research/RESEARCH-uiux-overhaul.md) — UI/UX overhaul research *(stale — file not found in current environment; gitignored local-only)*
    - [Research/RESEARCH-proof-of-bonsai-effectiveness.md](../../Research/RESEARCH-proof-of-bonsai-effectiveness.md) — OSS launch proof-of-work pre-registration (cut-over `4dfd3f4` 2026-04-14). Pick up when ready — user answers §10 first. *(stale — file not found in current environment; gitignored local-only)*
