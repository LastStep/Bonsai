...
- **Local `go build`/`go test` miss `golangci-lint unused`.** Repo config is now v2 (`.golangci.yml` `version: "2"`, migrated Plan 20). Local `golangci-lint run` requires v2.x binary — install via `go install github.com/golangci/golangci-lint/cmd/golangci-lint@v2.11.4`. Trust CI for dead-code on deletion/refactor PRs if local lint not installed.
...
- **Foundational research docs** — Anchor for methodology/concept decisions. (stale — `station/Research/` directory does not exist in this environment; files not found in git history. Marked 2026-05-06 memory-consolidation.)
