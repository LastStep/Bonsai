...
- **Local `go build`/`go test` miss `golangci-lint unused`.** Repo config is v2 (`.golangci.yml` `version: "2"`). Trust CI for dead-code on deletion/refactor PRs. If local lint version mismatches binary, install matching: `go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest`.
...
- **Foundational research docs** — Anchor for methodology/concept decisions. (stale — Research/ directory does not exist in repo; links broken as of 2026-05-01 Doc Freshness Check; awaiting user decision: restore files or remove references)
