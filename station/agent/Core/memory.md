...
**Current task:** Idle. Last ship (2026-04-25): **Plan 32 — Plan 31+29 review followup bundle** (PR #80 squash `99e9948`). Tier-1 knock-off bundle, single dispatch, 6 phases, 13 commits squashed. **Phase A** (`20a8f76`) extracted `internal/wsvalidate/` package (`InvalidReason` + `Normalise`); migrated `addflow/ground.go` (`invalidWorkspaceReason`) + `initflow/vessel.go validate()` to shared validator. **Phase B** (`7c792c1`) added backslash + pure-root rejection in `wsvalidate.InvalidReason` + 6 tests. **Phase C** (`053c6bc`) strengthened `TestConflicts_ColorTonesDifferPerAction` (Keep-vs-Backup, not just Keep-vs-Overwrite), positive companions `TestVessel_AcceptsCleanRelative` + `TestGround_AcceptsNestedRelative`, renamed `shortName` → `conflictsShortName`. **Phase D** (`d7b6fde`) collapsed `hasAbility` → `slices.Contains` (stdlib Go 1.21+); collapsed `agentsToSlice` + `requiredToSlice` → single `compatToSlice(a, omitEmpty bool)`. **Phase E** (`12707d0`) added `TestWriteCatalogSnapshot_TrailingNewline` + `TestSerializeCatalog_VersionPassThrough` (table: `""`, `"dev"`, `"v0.3.0"`). **Phase F** (`42252b4`) added `ProjectConfig.Validate()` chokepoint (wired into `Load()`, validates ProjectName + each agent.Workspace + DocsPath via wsvalidate, scans for 9 forbidden shell metachars `"`, `` ` ``, `$`, `\`, newline, `]`, `)`, `[`, `(`); replaced `os.WriteFile` with `os.OpenFile|syscall.O_NOFOLLOW` symlink-resistant write at `WriteCatalogSnapshot` + `TestWriteCatalogSnapshot_RefusesSymlink` regression. Plus lint-fix commit (`bd9b12f`) for `_ = f.Close()` errcheck + gofmt blank line. Closes 13 of 17 review items: Plan-29-cosmetic 1/2/3, Plan-29-test-gap 1, Plan-29-security-hardening 1/2/3, Plan-31-cosmetic 4/5, Plan-31-test-gap 1/2, Plan-31-security-hardening 1/2. Out of scope (filed remaining): Plan-29-test-gap 2 (`BodyOnlyDropsChrome` inverse), Plan-29-security-hardening 4 (Unicode lookalikes), Plan-31-cosmetic 1/2/3/6, Plan-31-security-hardening 3/4. **Process:** dispatch agent #1 crashed on rate limit mid-Phase-C with 2 commits + uncommitted-passing tests; continuation agent shipped Phase C-F + 4 commits clean. Code review PASS-WITH-MINORS (Co-Authored-By trailer split A+B vs C-F, O_NOFOLLOW final-component-only — both acceptable). Initial CI lint failed (errcheck on Close + gofmt blank), pushed 1 fix commit, all 6 CI green post-fix. Worktree cleaned. Plan archived. **Prior shipped work:** see `Playbook/Status.md` Recently Done for rolling window, `Playbook/StatusArchive.md` for older, and `Playbook/Plans/Archive/` for per-plan specs + details.
