...
**Current task:** [Plan 41 — Headless CLI Contract + MCP-ready cores](../../Playbook/Plans/Active/41-headless-cli-contract.md) **GRILLED + READY** (`0785904`, status:ready). Tier-2, 5 phases: headless cores for all 4 mutating cmds returning pure `*Result` + `list --json` + unified exit/event contract, shaped so MCP is a thin wrapper. **Next: dispatch Phase 1 SOLO** (changes RunInit/RunAdd sigs → must MERGE before 2/3/4 branch — no harness gate, manual discipline), then 2/3/4 parallel (file-disjoint, verified), 5 last. **Decided in grill:** keep minimal Result seam (Option C); error-unless-`--from` for multi-owner remove; drop `--json` from mutating cmds (read cmds keep it). **Capture init/add golden JSONL from `main` BEFORE the Phase-1 refactor** (byte-identity oracle for Bonsai-Eval rung-3). Supersedes Plan 40 P4 update-delivery; MCP server = fast-follow **Plan 42** (official go-sdk, GA v1.6.1, stdio `bonsai mcp`). Plan 40 P1-3 still untagged/tag-held; dogfood still also needs `.bonsai-lock.yaml` gitignore policy (Backlog P2).
