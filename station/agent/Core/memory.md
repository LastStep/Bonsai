...
- **Local `go build`/`go test` miss `golangci-lint unused`.** (partially stale — repo config migrated to v2 in Plan 20; `.golangci.yml` now `version: "2"`. The v1/v2 version mismatch error no longer applies. If running local golangci-lint, ensure a v2.x binary matches the config.) Trust CI for dead-code on deletion/refactor PRs.
...
- **Foundational research docs** — Anchor for methodology/concept decisions. (stale — `station/Research/RESEARCH-*.md` paths verified missing 2026-06-13; `Research/` directory does not exist in repo. Files may have been deleted or never created. User should confirm location or remove this reference block.)
