...
- **Local `go build`/`go test` miss `golangci-lint unused`.** Local golangci-lint v2.x; repo config is v1 → `golangci-lint run` errors locally with "unsupported version". Trust CI for dead-code on deletion/refactor PRs, OR install v1 locally (`go install github.com/golangci/golangci-lint/cmd/golangci-lint@v1.64.8`). (stale — `.golangci.yml` migrated to `version: "2"` and CI now pins `golangci-lint-action@v9` + `v2.11.4`; local/CI version mismatch no longer applies. Kept for historical context.)
...
- **Foundational research docs** — Anchor for methodology/concept decisions. (stale — `station/Research/` directory does not exist as of 2026-05-12 memory-consolidation audit; all 6 files below are unresolvable. Kept for historical context; remove or re-link when Research/ is re-created.)
