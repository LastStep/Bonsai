...
- **Foundational research docs** — Anchor for methodology/concept decisions. *(stale — Research/ directory absent from disk; files not found at `/home/user/Bonsai/Research/`. Were confirmed present as of 2026-05-07; likely lost in environment migration from `/home/rohan/ZenGarden/Bonsai`. Files are gitignored per `.gitignore` — check local disk of original dev environment or git history before the `.gitignore` addition if needed. Flagged: 2026-08-08 memory-consolidation.)*
