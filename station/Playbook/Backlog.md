...
- **[Plan-31-cosmetic] PR #75 review minors** — non-blocking, low-value. (1) `internal/generate/catalog_snapshot.go:192, 209` uses legacy `0755`/`0644` octal not Go-1.13 `0o755`/`0o644` prescribed in plan; neighbors use legacy octal so local convention wins, leaving for awareness. (2) Same file — `WriteCatalogSnapshot` duplicates the "read-existing + bytes.Equal + ActionUnchanged" logic already present in `writeFile:297-300`. Could be `writeFileNoLock` helper but current inline readable enough; skip unless third caller needs it. (3) `bonsai_reference_test.go` — rendered Markdown table emits `[path](path)` where link-text = URL; neighboring Core/How-to-Work tables use human-readable labels. Cosmetic fix: `[bonsai-model.md](relative-path)` or prose label. (4) `generate.go:1600-1607` — `hasAbility` is a 6-line linear-search helper where `slices.Contains` (Go 1.21+) would be one line. (5) `catalog_snapshot.go:82,96` — `agentsToSlice` vs `requiredToSlice` differ only in empty-list return (`[]string{}` vs `nil`); could be one flag-taking function. (6) Every `bonsai add <ability>` pays the cost of 6×3 template re-renders per peer just to emit `ActionUnchanged`. Low-cost; worth a benchmark if dogfooding feels slower. *(added 2026-04-24, source: PR #75 review)*
- **[Plan-31-test-gap] PR #75 coverage gaps** — non-blocking. (1) `catalog_snapshot_test.go` — no test that JSON output has trailing newline (asserted in source at line 189 via `append(data, '\n')`); trivial `bytes.HasSuffix` check. (2) Same file — no test that `""` or `"dev"` (the default for `go install`) flows to `snap.Version` correctly. Current test uses `"test-1.0"`. *(added 2026-04-24, source: PR #75 review)*
- **[Plan-31-security-hardening] PR #75 defence-in-depth** — non-blocking, all PRE-EXISTING weaknesses (not PR75 regressions). (1) `internal/config/config.go:49` `ProjectConfig.Load()` is raw YAML unmarshal with no schema enforcement — hand-edited `.bonsai.yaml` with `workspace: "../../etc/"` bypasses TUI's `invalidWorkspaceReason` validation (addflow/ground.go:289 + initflow/vessel.go:115). PR75's `RefreshPeerAwareness` + `bonsaiReferenceLines` merely exercise the same paths more often. Fix: add `ProjectConfig.Validate()` covering workspace, docs_path, agent-type names; reject shell-metachars (`"`, `` ` ``, `$`, `\`, newline, `]`, `)`, `[`, `(`). (2) `internal/generate/catalog_snapshot.go:209` `os.WriteFile` follows symlinks — attacker with project-root write access could pre-create `.bonsai/catalog.json` as symlink to `~/.ssh/authorized_keys` etc. Same class as every other `writeFile`/`writeFileChmod` in the generator (`generate.go:285, 303`) — not PR75-introduced. Fix: `os.OpenFile(abs, O_WRONLY|O_CREATE|O_TRUNC|syscall.O_NOFOLLOW, 0644)` or `lstat` reject on symlinks. (3) TOCTOU on `.bonsai/` dir perms — `os.MkdirAll(..., 0755)` silently succeeds if dir exists with different mode; Go doesn't chmod pre-existing dirs. Minor (contents non-secret). (4) Markdown link injection via tampered `cfg.DocsPath` — if `.bonsai.yaml` sets `docs_path: "x) [evil](http://attacker"` the rendered CLAUDE.md contains broken/fake link. Same root cause as (1). *(added 2026-04-24, source: PR #75 security review)*
