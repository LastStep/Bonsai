...
- **[bug] `writeFileChmod` skips chmod on `ActionUnchanged`** `[Group B]` — In `internal/generate/generate.go:292`, `writeFileChmod` only re-applies perms when the inner `writeFile` result is `ActionCreated|Updated|Forced`. With Plan 12's content-equality short-circuit, an identical-content run returns `ActionUnchanged`, so chmod is skipped. If a sensor script's execute bit was stripped externally (e.g., `chmod -x`) but its content is untouched, `bonsai update` would report "Up to date" and leave the file non-executable. Fix: add `ActionUnchanged` to the chmod gate, or re-chmod unconditionally when `result.perm != 0`. Narrow edge case — low priority. *(added 2026-04-17, source: PR #20 post-merge review)*
