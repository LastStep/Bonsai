...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED by v0.4.3 (shipped 2026-05-13, PR #105/#106). Hook commands now bake install-time absolute paths. Removed from Backlog by backlog-hygiene 2026-08-07. -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags [Plan 38 P2 blocker]" — RESOLVED by v0.4.2 (shipped 2026-05-13, PR #102). --non-interactive + --from-config flags added to init + add. Removed from Backlog by backlog-hygiene 2026-08-07. -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (shipped 2026-06-16). All four mutating cmds (init/add/update/remove) have headless cores + JSONL/exit contract. list --json added. Removed from Backlog by backlog-hygiene 2026-08-07. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry — ROTATION OVERDUE** `[URGENT 2026-08-07]` — Fine-grained PAT rotated 2026-04-22 (90-day default). Rotation deadline ~2026-07-15 has **passed** (3+ weeks overdue as of 2026-08-07). PAT may be expired — verify immediately before next release or GoReleaser brew step will fail with 401. Rotate via GitHub → Settings → Developer settings → Fine-grained PATs, then `gh secret set HOMEBREW_TAP_TOKEN --repo LastStep/Bonsai`. *(added 2026-04-22, urgency flag added 2026-08-07, source: v0.2.0 release session)*
