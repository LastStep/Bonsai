...
<!-- [bug] Sensor hook commands use $PWD-walk-up — RESOLVED: v0.4.3 hotfix (2026-05-13) baked absolute install-time project root into each hook command. Status.md confirms ship. (resolved 2026-07-03 backlog-hygiene) -->
...
<!-- [feature] bonsai init / bonsai add need non-interactive flags — RESOLVED: v0.4.2 (2026-05-13) shipped --non-interactive + --from-config flags with JSONL stdout and exit codes 0/2/3/4. Unblocked Plan 38 P2 rung-3. (resolved 2026-07-03 backlog-hygiene) -->

## P1 — High

<!-- [feature] Full agent-drivable (non-interactive) CLI parity — RESOLVED: Plan 41 (2026-06-16) shipped headless *Result cores + JSONL/exit-code contract (ExitConflict=5) for all 4 mutating cmds (init/add/update/remove) plus list --json. contract doc at docs/agent-interface.md. (resolved 2026-07-03 backlog-hygiene) -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[ACTION NEEDED — due ~2026-07-15, 12 days away]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — rotate before ~2026-07-15. Same applies to any other PATs in repo secrets (audit + document expiry dates). Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds (binaries published, only formula update missed). *(added 2026-04-22, source: v0.2.0 release session; flagged urgent 2026-07-03 backlog-hygiene)*
