...
- **[ops] Add Windows cross-compile gate to `ci.yml`** — Plan 32 (PR #80) added `syscall.O_NOFOLLOW` (POSIX-only) and broke Windows cross-compile. CI runs Linux-only `go build`/`go test`/`govulncheck` so the bug went undetected until v0.4.0 GoReleaser run. Add `GOOS=windows GOARCH=amd64 go build ./...` step to the test or lint job — fast (<10s), catches this class permanently. *(added 2026-05-04, source: v0.4.0 ship — hotfix PR #95)*
...
- **[improvement] Better trigger sections — clearer activation conditions for catalog items** — Last unchecked Phase 1 Roadmap item. Catalog abilities (`meta.yaml`) lack explicit activation conditions beyond `agents:` compatibility. Skills/workflows need `trigger:` or `activation:` metadata so agents know when to auto-load them. Enables more reliable context-guard phrase matching and auto-invocation. Scoped under Plan 08 Phase C3 (prompt hook intent classification) but that research item is deferred — this is the broader catalog-side work. *(added 2026-04-21 per routine-digest note, re-captured 2026-05-04 backlog-hygiene — was listed in routine-digest log but never added to Backlog)*
