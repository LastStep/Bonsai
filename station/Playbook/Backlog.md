...
<!-- [bug] Sensor hook commands use $PWD-walk-up — RESOLVED via v0.4.3 (PR #105/#106, 2026-05-13). Hook commands now bake install-time absolute paths. -->
...
<!-- [feature] bonsai init / bonsai add need non-interactive flags — RESOLVED via v0.4.2 (PR #102, 2026-05-13). --non-interactive + --from-config shipped for init + add. Full headless contract (all 4 cmds) shipped via Plan 41. -->

## P1 — High

<!-- [feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove — RESOLVED via Plan 41 (PRs #120/#122/#123/#121/#125, 2026-06-16). Every mutating cmd now has a pure *Result headless core + JSONL/exit contract. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[⚠ DUE ~2026-07-15 — 20 days]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — set calendar reminder for ~2026-07-15 to rotate before next release. Same applies to any other PATs in repo secrets (audit + document expiry dates). Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds (binaries published, only formula update missed). *(added 2026-04-22, source: v0.2.0 release session)*
