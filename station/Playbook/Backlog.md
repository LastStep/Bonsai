...
- **[feature] Workspace artifact sync on add/update/remove** — When catalog items are added, updated, or removed via `bonsai add`/`bonsai update`/`bonsai remove`, workspace artifacts like `CLAUDE.md` navigation tables and `INDEX.md` should be regenerated to reflect the change. Currently, user-modified `CLAUDE.md` is skipped by the lock file conflict system, so new items don't appear in the navigation table. Needs a smarter approach — either section-based merging (only update the managed navigation sections, preserve user content) or a post-generation fixup pass that inserts/removes entries without overwriting the whole file. The `<!-- BONSAI_START/END -->` markers already exist for this purpose. *(added 2026-04-15, source: user)*
