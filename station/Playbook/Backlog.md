...
<!-- "CHANGELOG.md + richer release notes" (2026-04-20) — merged into Group D "Changelog generation skill + release changelogs" 2026-04-21 (backlog hygiene): the Group D item already covers the CHANGELOG.md artifact + v0.1.0-v0.1.3 backfill + future-release wiring. -->
<!-- "Consolidate or delineate CHANGELOG backlog items" (2026-04-21) — resolved by the merge above, 2026-04-21 (backlog hygiene). -->
...
- **[feature] Changelog generation skill + release changelogs** — One coherent item covering both the artifact and the tooling. Add a changelog generation skill that: (1) parses conventional commit messages between tags to generate structured changelogs, (2) outputs `CHANGELOG.md` at repo root in keep-a-changelog format, (3) generates release notes for `gh release create --notes` (replacing the auto-generated `"style: apply gofmt -s..."`-style notes that don't signal a maintained project). Backfill v0.1.0–v0.1.3 by scraping commit history between tags. Wire future releases to update `CHANGELOG.md`. Also consider a `bonsai changelog` CLI command. *(added 2026-04-16, merged with Group C CHANGELOG item 2026-04-21, source: user + backlog hygiene)*
...
- **[improvement] Plan archiving — Active/Archive folder structure + Plans Index** — Plans currently all live in `Plans/Active/`. Completed plans should move to `Plans/Archive/` after merge, and a `Plans/INDEX.md` listing active/archived plans with one-line summaries should be generated or hand-maintained. Requires: create `Plans/Archive/` in scaffolding manifest, add `Plans/INDEX.md` to scaffolding, update issue-to-implementation workflow (Phase 10) to move the plan + refresh the index, update planning workflow and planning-template skill, update session-start protocol if it scans for active plans, update CLAUDE.md nav table. *(added 2026-04-16, merged with standalone Plans Index item 2026-04-21, source: user + backlog hygiene)*
<!-- "Root Bonsai/CLAUDE.md project-structure tree drift" (2026-04-21) — fixed 2026-04-21 on branch claude/get-started-PlBaC (backlog hygiene): internal/tui/ block expanded to include styles_test.go + harness/ subtree (harness.go, harness_test.go, steps.go, steps_test.go). -->
<!-- "Plans Index file" (2026-04-21) — folded into "Plan archiving" item above 2026-04-21 (backlog hygiene). -->
...
> Dogfooding findings from the 2026-04-17 `bonsai init` walk-through. Mostly `init` flow polish — prompt flow ergonomics and the review→generate→complete flow. Palette tokens and banner shipped via Plan 11/14; AltScreen lifecycle shipped via Plan 15. Remaining items are independent — tackle as a coherent design pass or piecemeal.

<!-- "Define a canonical color palette for the whole TUI" — shipped 2026-04-17 via Plan 11 / PR #14 (Phase 1 adaptive palette + semantic tokens in internal/tui/styles.go) and further refined in Plan 14 / PR #24 (Phase 3). -->
<!-- "Redesign the B O N S A I / agent scaffolder banner" — shipped 2026-04-17 via Plan 14 / PR #24 (Phase 3 visual identity). -->
...
<!-- "Persist answered prompt values on screen through multi-step init" — shipped 2026-04-17 via Plan 14 / PR #24 (answered-prompt persistence). -->
...
<!-- "Show counts alongside ability category headers" — shipped 2026-04-17 via Plan 12 / PR #20 (ItemTree `(N)` counts in review panel). Closed as done 2026-04-21 (backlog hygiene); file a fresh item if further category polish surfaces. -->
<!-- "TUI screen lifecycle — clear prior step output on major transitions" — resolved 2026-04-20 via Plan 15 / PR #26: bonsai init + add (+ remove/update) now run in AltScreen (cmd/init.go:146, cmd/add.go:149), which gives each major transition a clean canvas as the item requested. -->
...
<!-- "[security] Bump golang.org/x/net v0.38.0 → v0.45.0+" (2026-04-21) — merged into batch bump item below 2026-04-21 (backlog hygiene). x/net is just one of ~17 stale modules; handle as one sweep. -->
- **[security] Batch refresh outdated Go modules** — 17+ modules behind per `go list -m -u all` 2026-04-21: `golang.org/x/net v0.38 → v0.45+` (clears GO-2026-4441 infinite-parse loop + GO-2026-4440 quadratic `x/net/html` parse — both unreachable today but easy hygiene), `golang.org/x/crypto v0.36 → v0.50`, `x/tools v0.37 → v0.44`, `x/sys v0.38 → v0.43`, `x/text v0.30 → v0.36`, `x/mod v0.28 → v0.35`, `uax29/v2 v2.5 → v2.7`, `goldmark v1.7.13 → v1.8.2`, plus charm/bubbletea ecosystem. Hygiene sweep: `go get <pkg>@latest && go mod tidy && govulncheck ./...` after Go 1.25 toolchain bump (already landed via Plan 20). Promoted from P3 and merged with the x/net-specific item 2026-04-21. *(added 2026-04-21, source: routine-digest + backlog hygiene)*
...
<!-- "[debt] Batch refresh outdated Go modules" (2026-04-21) — promoted to P2 and absorbed into ungrouped-P2 batch bump item 2026-04-21 (backlog hygiene). -->
