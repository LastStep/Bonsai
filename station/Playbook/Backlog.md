...
- **[improvement] Consolidate or delineate CHANGELOG backlog items** — Group C "CHANGELOG.md + richer release notes" and Group D "Changelog generation skill + release changelogs" have overlapping scopes. Decide: merge into one item, or explicitly split into (C) the CHANGELOG.md artifact + v0.1.0–v0.1.3 backfill vs (D) the generation tooling/skill + `bonsai changelog` CLI. Line 91 already notes overlap — make it explicit. *(added 2026-04-21, source: routine-digest)*
...
- **[docs] Root `Bonsai/CLAUDE.md` project-structure tree drift** — `internal/tui/` block in the root CLAUDE.md is missing the Plan 15 `harness/` subdirectory (7 files: harness/steps/splicer/reducer/dialogs/etc) and `styles_test.go`. Plan 15 merged 2026-04-20; tree was not updated. Needs edit outside station/ scope — dispatch a code agent or user edits directly. Suggested replacement block in `Reports/Pending/2026-04-21-doc-freshness-check.md` item 3. *(added 2026-04-21, source: routine-digest)*
