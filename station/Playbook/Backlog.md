...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED via v0.4.3 hotfix (Status.md Recently Done 2026-05-13). Removed by backlog-hygiene 2026-08-20. -->
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED via v0.4.2 (Status.md Recently Done 2026-05-13). Removed by backlog-hygiene 2026-08-20. -->
...
_(No open P0 items — see Status.md for recently resolved items.)_

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity" — RESOLVED via Plan 41 (all 4 cmds headless, JSONL/exit contract, Status.md Recently Done 2026-06-16). Removed by backlog-hygiene 2026-08-20. -->
...
- **[security] Fix 4 active Go CVEs — upgrade stdlib to go1.25.13 + x/text + goldmark** — `govulncheck` reports 4 vulns actively reachable in the built binary: GO-2026-6218 (`net/url` quadratic, fixed go1.25.13), GO-2026-6088 (`encoding/xml` recursion, fixed go1.25.13), GO-2026-5970 (`golang.org/x/text` v0.36→v0.39.0, infinite loop), GO-2026-5320 (`goldmark` v1.7.13→v1.7.17, XSS). Fix: bump Go toolchain to `go1.25.13` in `go.mod` + `ci.yml`, then `go get golang.org/x/text@v0.39.0 github.com/yuin/goldmark@v1.7.17 && go mod tidy`. Confirmed failing on `main` — not a regression of this PR. *(added 2026-08-20, source: bonsai-loop govulncheck)*
...
- **[security] Website npm vuln tree — astro upgrade breaks `npm run build`** — 6 open Dependabot alerts on `/website` (esbuild HIGH→0.28.1, vite HIGH+MED→7.3.5, js-yaml MED→4.2.0, astro+esbuild LOW). All transitive via astro/starlight (Go binary govulncheck — 4 CVEs now active, see P1 security entry added 2026-08-20). devalue HIGH already cleared (PR #109). The astro 6.1.7→6.3.2 bump (PR #108) that would clear most **fails the website build** after rebase — needs a real upgrade-with-build-fix pass, not a dependabot auto-merge. This is the overdue **vulnerability-scan routine's** job. Either fix #108's build break + bump vite/js-yaml, or pin patched transitives. *(added 2026-06-16, source: "fix all issues" sweep during Plan 41 dispatch)*
