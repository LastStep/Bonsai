...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED by v0.4.3 hotfix (2026-05-13, PRs #105/#106). Absolute install-time project root now baked into each hook command in .claude/settings.json. Existing projects need `bonsai update` to refresh. Removed from active backlog 2026-09-07 (backlog-hygiene routine). -->
...
<!-- "[feature] bonsai init / bonsai add non-interactive flags" — RESOLVED by v0.4.2 (2026-05-13, Plan 39). --non-interactive --from-config shipped for both cmds; JSONL stdout + exit codes 0/2/3/4. Unblocked Plan 38 P2 rung-3. Removed from active backlog 2026-09-07 (backlog-hygiene routine). -->

## P1 — High

<!-- "[feature] Full agent-drivable CLI parity: init/update/add/remove" — RESOLVED by Plan 41 (2026-06-16, PRs #120-#125, main ab202c3). All mutating cmds have pure *Result headless cores + JSONL/exit contract (ExitConflict=5); list --json; docs/agent-interface.md contract doc. Removed from active backlog 2026-09-07 (backlog-hygiene routine). -->

- **[security] Upgrade Go toolchain + deps to clear 4 govulncheck findings** — `go1.25.9` needs bumping to `go1.25.13` (fixes GO-2026-6218 `net/url` quadratic complexity, GO-2026-6088 `encoding/xml` recursion depth); `golang.org/x/text` needs `v0.39.0` (GO-2026-5970 infinite loop); `github.com/yuin/goldmark` needs `v1.7.17` (GO-2026-5320 XSS). All 4 are active traces through production code paths (`guideflow`, `cmd/bonsai/main.go`). Surfaced by govulncheck CI on PR #233 — confirmed pre-existing on main. Fix: `go get golang.org/x/text@v0.39.0 github.com/yuin/goldmark@v1.7.17`, update `go` directive to `go 1.25.13`, `go mod tidy`. *(added 2026-09-07, source: govulncheck CI / routine maintenance run)*
...
- **[ops] Routine bot PR pile-up — eliminate parallel-track artifacts** — Cloud-routine cron pushes daily `claude/bonsai-maintenance-<date>` PRs containing routine reports (RoutineLog, Reports/Pending, memory.md, routines.md). Local routine-digest runs directly on main; bot PRs accumulate without merge. 9 stale PRs closed 2026-05-07 (#86–#91, #96–#98). Fix: change cloud routine to either (a) commit-direct-to-main, (b) auto-merge if green + no conflicts, or (c) skip PR creation when local digest has already absorbed the date range. *(added 2026-05-07, source: PR triage)*
...
- **[improvement] Plan 40 review nits (non-blocking)** — (1) manifest `created` field emitted bare (`created: {{ .Created }}`), not via `yamlScalar`; re-inject only possible if attacker already controls `.bonsai/project.yaml` (trust boundary already crossed, injected key schema-ignored) — route through `yamlScalar` for uniformity (note: would quote-wrap the date, update deterministic-bytes test). (2) Stale doc-comment `internal/generate/generate.go` ~L388 says "`manifestRel`" but const is `manifestTmplRel`. (3) Phase 2: note `valid_from` is parsed but never format-validated despite the frozen schema's "`YYYY-MM-DD` if present"; `dateCharset` is applied to manifest `created` only — add a one-line guard in `internal/validate/project.go`. *(added 2026-06-13, source: Plan 40 PR #114/#116 reviews)*
- **[bug] `bonsai validate` can't pass on the Bonsai repo itself — `.bonsai-lock.yaml` is gitignored (`.gitignore:15`)** — with no committed lock, validate reports every installed ability as `orphaned_registration` (36 errors + 2 untracked-custom warnings on the current tree). Surfaced 2026-06-13 attempting the Plan 40 dogfood: the scaffold added zero new issues but the pre-existing orphan wall makes the dogfood's "validate exits 0" gate unreachable. Decide lock-file policy for the self-hosted station (commit the lock + `bonsai update` once to re-lock, vs. accept validate-unusable-here). Blocks proper Plan 40 dogfood. *(added 2026-06-13, source: Plan 40 dogfood attempt)*
- **[security] Website npm vuln tree — astro upgrade breaks `npm run build`** — 6 open Dependabot alerts on `/website` (esbuild HIGH→0.28.1, vite HIGH+MED→7.3.5, js-yaml MED→4.2.0, astro+esbuild LOW). All transitive via astro/starlight (Go binary unaffected — govulncheck clean). devalue HIGH already cleared (PR #109). The astro 6.1.7→6.3.2 bump (PR #108) that would clear most **fails the website build** after rebase — needs a real upgrade-with-build-fix pass, not a dependabot auto-merge. This is the overdue **vulnerability-scan routine's** job. Either fix #108's build break + bump vite/js-yaml, or pin patched transitives. *(added 2026-06-16, source: "fix all issues" sweep during Plan 41 dispatch)*
- **[debt] Unify `remove` business logic — cinematic path duplicates the headless cores** — Plan 41 Phase 3 (PR #123) shipped `nonint.RunRemove{Agent,Item}` as the headless impl but left the cinematic `cmd/remove.go` flow's in-cmd target-resolution intact (`filterRequired` lives in both `cmd/remove.go` and `nonint.filterRequiredItem`). Plan asked the cinematic path to call the cores; agent kept them separate because the cores self-`lock.Save` and expose no hook for the interactive conflict picker. Drift risk between the two resolvers. Consolidate (cores split compute-from-commit so the TTY path can route through them), or accept + add a contract test pinning both paths to identical owner/required decisions. *(added 2026-06-16, source: Plan 41 Phase 3 review)*
...
- **[Plan-29-test-gap] Inverse-chrome companion test** — `TestGenerateStage_BodyOnlyDropsChrome` doesn't verify chrome IS present when `SetBodyOnly(false)`. Add positive-chrome companion. *(added 2026-04-23, source: [PR #72](https://github.com/LastStep/Bonsai/pull/72) review; item 1 closed 2026-04-25 via Plan 32)*
...
- **[Plan-31-cosmetic] PR #75 review minors (4 remaining)** — `catalog_snapshot.go`: legacy `0755`/`0644` octal vs `0o755` (1), `WriteCatalogSnapshot` duplicates `writeFile:297` ActionUnchanged logic (2), peer-render benchmark gap on `bonsai add` (6). `bonsai_reference_test.go`: table emits `[path](path)` vs neighbor human-label convention (3). *(added 2026-04-24, source: [PR #75](https://github.com/LastStep/Bonsai/pull/75) review; items 4/5 closed 2026-04-25 via Plan 32)*
- **[Plan-31-security-hardening] TOCTOU on `.bonsai/` dir perms** — `os.MkdirAll(.bonsai/, 0755)` silently succeeds with different mode on existing dir; Go doesn't chmod pre-existing. Minor (contents non-secret). *(added 2026-04-24, source: [PR #75](https://github.com/LastStep/Bonsai/pull/75) security review; items 1/2/4 closed 2026-04-25 via Plan 32)*
