...
<!-- [bug] Sensor hook commands use $PWD-walk-up — RESOLVED 2026-05-13 in v0.4.3 (PR #105/#106); hook commands now bake install-time absolute paths. Cleaned up 2026-08-15 backlog-hygiene. -->
...
<!-- [feature] bonsai init / bonsai add need non-interactive flags — RESOLVED 2026-05-13 in v0.4.2 (PR #102); --non-interactive --from-config shipped. Cleaned up 2026-08-15 backlog-hygiene. -->

_(P0 section is currently empty — no critical issues.)_

## P1 — High

<!-- [feature] Full agent-drivable CLI parity (init/update/add/remove) — RESOLVED 2026-06-16 via Plan 41 (PRs #120-#125); all four mutating commands have headless cores + JSONL/exit contract. Cleaned up 2026-08-15 backlog-hygiene. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[OVERDUE — rotate now]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — rotation was due ~2026-07-15 (now ~1 month overdue as of 2026-08-15). PAT may already be expired; next release Homebrew step will fail with `401 Bad credentials` if so. Rotate at `github.com/settings/personal-access-tokens` and update secret via `gh secret set HOMEBREW_TAP_TOKEN --repo LastStep/Bonsai`. *(added 2026-04-22, source: v0.2.0 release session; escalated 2026-08-15 backlog-hygiene)*
