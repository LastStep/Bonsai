...
<!-- [bug] Sensor hook commands use $PWD-walk-up — RESOLVED via v0.4.3 (PR #105/#106, 2026-05-13). Hook commands now bake install-time absolute paths. Existing projects need `bonsai update` to refresh. Removed from P0 2026-06-24 backlog-hygiene routine. -->
...
<!-- [feature] `bonsai init` / `bonsai add` need non-interactive flags — RESOLVED via v0.4.2 (Plan 39, PR #102, 2026-05-13). Both `--non-interactive` and `--from-config` flags shipped. Removed from P0 2026-06-24 backlog-hygiene routine. -->

## P1 — High

- **[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove** `[Plan 41 partial]` — Odysseus hub + AI agents must drive Bonsai headless, no TUI. **Plan 41 (2026-06-16) shipped:** all four cmds (init/add/update/remove) now have pure `*Result` headless cores + JSONL/exit contract (ExitConflict=5); `list --json` added; `docs/agent-interface.md` contract doc shipped. **Remaining:** MCP server (fast-follow Plan 42 per Status.md). Review Plan 42 readiness and promote to plan or close this entry. *(added 2026-06-13, source: user — "main thing"; updated 2026-06-24 backlog-hygiene — Plan 41 delivered core ask)*

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[URGENT — expires ~2026-07-15, 21 days away]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — set calendar reminder for ~2026-07-15 to rotate before next release. Same applies to any other PATs in repo secrets (audit + document expiry dates). Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds (binaries published, only formula update missed). *(added 2026-04-22, source: v0.2.0 release session; flagged urgent 2026-06-24 backlog-hygiene — rotate by 2026-07-15)*
