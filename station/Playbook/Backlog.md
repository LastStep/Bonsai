...
- **[bookkeeping] Re-archive Plan 29 file** `[housekeeping]` — `station/Playbook/Plans/Active/29-init-add-bug-bundle.md` still sits in `Plans/Active/` after Plan 29 shipped (PR #72 squash `9eb2bff`, 2026-04-23). My archive move got absorbed by parallel session's Plan 28 close-out commit (`c8d66f8`), then reverted by parallel session's follow-up (`c758945` — "revert accidental Plan 29 archive") because from their view it looked like an inadvertent index touch. Plan 29 is complete per Status.md Recently Done row + memory.md Work State. Next session: confirm no other parallel-session in-flight work on the file, then `git mv` to `Plans/Archive/`. *(added 2026-04-23, source: Plan 29 wrap-up)*
...
- **[Plan-29-cosmetic] init+add bug bundle review minors** — PR #72 review minors (all non-blocking, low-value). (1) `internal/tui/addflow/conflicts_test.go` — `TestConflicts_ColorTonesDifferPerAction` only asserts Keep vs Overwrite strings differ, but the rendered row's action-LABEL text ("keep local" vs "overwrite") also differs, so the test would pass purely from label text even if palette were identical. Consider additionally comparing Keep vs Backup OR assert on raw ANSI-escape substrings. (2) Same file — `shortName` test helper collides semantically with any future production `shortName`; consider `conflictsShortName` or `testShortName` rename. (3) `internal/tui/addflow/ground.go` invalid-workspace error "no leading /" is slightly misleading on Windows where `filepath.IsAbs` also catches `C:\...`. Rationale comment acknowledges Windows but user-facing string doesn't. *(added 2026-04-23, source: PR #72 review)*
- **[Plan-29-test-gap] happy-path validator coverage for Phase H** — (1) No test verifies ACCEPT case for `./foo` or `foo/../bar` (paths that `filepath.Clean`-reduce to safe relative paths). Happy-path rests on unmodified `TestNormaliseWorkspace`. Add `TestGround_AcceptsNestedRelative` + `TestVessel_AcceptsCleanRelative` to lock "did not over-reject". (2) `TestGenerateStage_BodyOnlyDropsChrome` doesn't verify inverse (chrome IS present when `SetBodyOnly(false)`). Add positive-chrome companion test or concrete pointer. *(added 2026-04-23, source: PR #72 review)*
- **[Plan-29-security-hardening] Phase H validator hardening** — PR #72 security review minors (all non-blocking, defence-in-depth polish). (1) `internal/tui/initflow/vessel.go:110-135` diverges from Ground by skipping `filepath.Clean` before segment scan — legit in-root path like `foo/..` (Cleans to `.`) is rejected by Vessel but accepted by Ground. Not a hole (Vessel fails-closed), but entry points should share one helper. Extract `invalidWorkspaceReason` into shared package, call from both. (2) Windows-style absolute paths (`C:\foo`, `\etc\passwd`) slip past POSIX `filepath.IsAbs` AND past `..` segment scan (backslash legal on POSIX → literal segment). Defensive `strings.ContainsRune(ws, '\\')` reject would save user from creating literal `C:\foo` directory when they meant Windows path. (3) `invalidWorkspaceReason` does not reject pure root result (`./` — what `.` / `foo/..` normalise to). Installs station at project root itself — legal but probably unintended. (4) Unicode lookalikes (`․․` U+2024, `．．` U+FF0E) pass as literal directory names. Filesystems don't resolve — no traversal — but NFKC normalisation before `..` scan would flag homoglyph attempts. Purely speculative. *(added 2026-04-23, source: PR #72 security review)*
- **[debt] Cross-agent `OtherAgents` template staleness on `bonsai add`** — `catalog/sensors/scope-guard-files/*.sh.tmpl` + `catalog/sensors/dispatch-guard/*.sh.tmpl` render `{{ range .OtherAgents }}`. On `bonsai add <new>`, `generate.AgentWorkspace` + Plan 27 PR1's new `*ForAgent` scope-narrowing only run for the new agent — already-installed OTHER agents' `scope-guard-files.sh` + `dispatch-guard.sh` are NOT re-rendered and lack the new agent in their `OtherAgents` list until `bonsai update`. Pre-existing scoping behaviour; not introduced by Plan 27 PR1 but surfaced by its security review. Fix: after `bonsai add`, call a light "regenerate sensors for other agents" pass scoped to just those two sensors, OR document the need to run `bonsai update` after each add. Non-blocking (functional correctness intact; just stale cross-agent awareness). *(added 2026-04-22, source: PR #67 security review)*
- **[cosmetic] Plan-28 Phase 1 review NITs [Plan-28 cosmetic]** — six nits from PR #68 code review, all low-impact polish in `internal/tui/catalogflow/browser.go`: (1) `timeZero()` wrapper unnecessary (inline `time.Time{}`, drop `time` import at single call site); (2) stale `filterHidZero` reference in `category` struct docstring (field doesn't exist); (3) redundant `expanded: false` in struct literal (zero value); (4) `Agents` section Entry records leave Agents/Required empty → details block renders "(no extra metadata)" — consider surfacing `Kind: AGENT` meta or the agent name for parity with other sections; (5) body padding/truncation math duplication partially resolved by MINOR 3's `Stage.RenderFrame` routing (path A) but residual unused helpers may remain — sweep for dead code; (6) no test asserting greyed `(0)` tabs render visually muted (count asserted, style not) — add a test that inspects rendered output for the grey style application on filtered tabs. Batch one-shot. *(added 2026-04-22, source: PR #68 code review)*
- **[cosmetic] Plan-28 Phase 2+3 post-fix NITs [Plan-28 cosmetic]** — residual low-value polish after fix-agent round. Phase 2: `itoa` helper at `internal/tui/listflow/agent_panel.go:378-398` duplicates `internal/tui/initflow/enso.go:250` (deliberate to avoid `strconv` import there; consolidating to a single `internal/tui/util/` or similar would trim ~20 lines across repo); no test for `workspace: ""` empty-string defensive branch at `renderWorkspaceBlock` line 161 (covered indirectly by happy path but a direct test would lock the contract); `cmd/list_test.go` covers only happy path, no e2e for empty-agents case or `terminalSize() → 80,24` non-TTY fallback. Phase 3: `chromeRows` magic constant `8` in `internal/tui/guideflow/viewer.go:243` fragile if header/footer row counts change — compute from `lipgloss.Height(header+footer)` like `Stage.RenderFrame` does; `labelFor` returns hyphen→space (`"CUSTOM FILES"`) while `shortFor` preserves space + truncates — minor inconsistency for future topic keys; `NewStage("GUIDE", ...)` label args are wasted since rail is hidden — `StageLabel{English:"GUIDE"}` + `title:"GUIDE"` only exist so `SetHeaderAction("GUIDE")` can immediately overwrite; `renderMarkdown` error path (`glamour.NewTermRenderer` failure) uncovered — hard to trigger with the auto-style config, acceptable to skip. Batch one-shot. *(added 2026-04-23, source: PR #70 + #71 code reviews)*
