...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED by v0.4.3 (2026-05-13). Hook commands now bake absolute install-time project root. Removed from P0 by backlog-hygiene 2026-08-06. -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED by v0.4.2 (2026-05-13). --non-interactive + --from-config shipped. Removed from P0 by backlog-hygiene 2026-08-06. -->

_(no active P0 items)_

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (2026-06-16). All four commands (init/add/update/remove) now have headless cores + JSONL/exit contract. Removed from P1 by backlog-hygiene 2026-08-06. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[PAST DUE — rotate now]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — set calendar reminder for ~2026-07-15 to rotate before next release. Same applies to any other PATs in repo secrets (audit + document expiry dates). Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds (binaries published, only formula update missed). *(added 2026-04-22, source: v0.2.0 release session)*
