...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED 2026-05-13 via v0.4.3 (PRs #105/#106). Hooks now bake install-time absolute paths. Existing projects need `bonsai update` to refresh. Removed from P0 2026-09-01 (backlog-hygiene). -->
...
<!-- "[feature] `bonsai init` / `bonsai add` need non-interactive flags" — RESOLVED 2026-05-13 via v0.4.2. `--non-interactive` + `--from-config <path>` shipped for both init and add. Removed from P0 2026-09-01 (backlog-hygiene). -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED 2026-06-16 via Plan 41 (PRs #120/#122/#123/#121/#125). All four mutating cmds now have headless cores + JSONL/exit-code contract (ExitConflict=5); `list --json`; `docs/agent-interface.md` contract doc. Removed from P1 2026-09-01 (backlog-hygiene). -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry — OVERDUE, rotate immediately** — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22; rotation was due ~2026-07-15. Today is 2026-09-01 — PAT is 47+ days past expiry. Next release will fail at the brew step with `401 Bad credentials`. Rotate now + document next expiry (~2026-12-01). *(added 2026-04-22, updated 2026-09-01 backlog-hygiene — PAT now expired)*
