...
- **[security] Bump `golang.org/x/net` v0.38.0 → v0.45.0+** — Clears GO-2026-4441 (infinite parsing loop) and GO-2026-4440 (quadratic parsing). Unreachable. **Scoped into Plan 36 release prep.** *(added 2026-04-21, source: routine-digest; reaffirmed 2026-05-04)*
- **[security] Bump Go toolchain 1.25.8 → 1.25.9** — Clears 6 unreachable stdlib CVEs (GO-2026-4864/4947/4946/4870/4869/4865). Hygiene-grade; gated on go1.25.9 release availability. **Scoped into Plan 36 release prep.** *(added 2026-05-04, source: routine-digest)*
- **[debt] Plan 36 docs sweep — ST-1 + ST-2 + PW-1** — Three doc-freshness items bundled into Plan 36 release prep: (a) INDEX.md architecture diagram refresh — TUI subsystem mentions BubbleTea + cinematic flows; add `internal/validate/` and `internal/wsvalidate/`; (b) `code-index.md` sweep — add `bonsai validate`, fix 5 stale line numbers, document 6 missing TUI packages; (c) root `Bonsai/CLAUDE.md` project-structure tree refresh — add `cmd/init_flow.go`, `cmd/validate.go`, `internal/validate/`, `internal/wsvalidate/`, 7 cinematic-flow `internal/tui/{flow}/` packages. *(added 2026-05-04, source: routine-digest)*
## P3 — Ideas & Research
