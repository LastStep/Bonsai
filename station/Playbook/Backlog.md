...
- **[bug] Silent error swallowing in spinner callbacks** `[Group B]` — ~30 `_ =` swallow sites remain in `cmd/` after the Plan 15 harness migration. The new `harness.SpinnerStep` (`internal/tui/harness/steps.go`) accepts `func() error` and propagates errors via `spinnerDoneMsg`, so the mechanism is in place — but callsites (`cmd/remove.go` 12, `cmd/update.go` 4, `cmd/init.go` 5, `cmd/add.go` 8, `cmd/root.go` 1) still discard individual errors inside spinner closures "for parity with legacy code" (see comment at `cmd/remove.go:runRemoveItemAction`). Fix: collect errors in an `[]error` inside each action closure and return `errors.Join(...)` so the harness surfaces them. *(added 2026-04-16, harness landed 2026-04-20 via PR #26, source: repo-analytics)*
...
- **[improvement] CHANGELOG.md + richer release notes** — Releases v0.1.0–v0.1.3 shipped with auto-generated notes like `"style: apply gofmt -s to all Go source files"` — doesn't signal a maintained project to evaluators. Add a `CHANGELOG.md` at repo root (keep-a-changelog format), backfill v0.1.0–v0.1.3 by scraping commit history between tags, and wire future releases to update it. May land alongside the "changelog generation skill" in Group D, or as a standalone. *(added 2026-04-20, source: session — OSS readiness review)*
- **[improvement] Run Astro build on PRs touching `website/`** — Currently the `CI` workflow (test + lint + GitGuardian) runs on `pull_request` but `Deploy Docs` only runs on `push` to main, so a broken MDX file passes PR CI and then fails on main. Hit 2026-04-20 with PR #25 (`<https://...>` autolink in `guide.mdx:20` — fixed in `e336ccb`). Fix options: (1) add a PR-triggered check job to `.github/workflows/docs.yml` that runs `npm run build` without deploying when `website/**` changes, or (2) add an `npm --prefix website run build` step to the existing PR `CI` workflow gated on a `paths` filter. Option 1 keeps the failure visible as "Deploy Docs" so the mental model matches. *(added 2026-04-20, source: session — PR #25 post-merge incident)*
...
- **[bug] `ConditionalStep` predicate not re-evaluated on Esc-back** — `internal/tui/harness/steps.go:794-797` docstring claims "If the user later Esc-backs and changes upstream picks, Reset() re-evaluates the predicate." In practice `Reset()` only zeros `skip`/`skipDone` and resets the inner step; the harness's Esc-back loop never re-Inits the popped-onto step, so the predicate fixes on first Init. No current caller hits the divergence (Conditional(Spinner) auto-completes past on second visit; Conditional(Confirm) wraps a stable selection bool), but a future caller that depends on predicate re-evaluation will silently get stale behavior. Fix: either capture `c.initPrev` in Reset and re-call `c.predicate()`, or rewrite the docstring to match actual behavior. *(added 2026-04-20, source: Plan 15 iter 3 review)*
- **[improvement] `NewConditional` should guard against nil predicate** — `internal/tui/harness/steps.go:807` accepts a nil predicate without checking; `c.predicate(c.initPrev)` panics in `Init` outside the harness's `recoverBuilder` scope (recovery only covers `Build`/`Splice` closures). Either add an explicit guard with a sensible default (treat nil as `true`) or assert at construction time. Same class as the spinner-action panic gap below. *(added 2026-04-20, source: Plan 15 iter 3 review)*
- **[improvement] Wrap SpinnerStep action goroutines in recover()** — `internal/tui/harness/steps.go:746-757` runs the action body inside a `tea.Cmd` closure; a panic there crashes the whole program mid-AltScreen with a stack dump (e.g., a nil-deref inside `runAddSpinner`/`runUpdate`'s spinner closure on a malformed lock file). Iter 3 closed the equivalent gap for `Build`/`Splice` via `recoverBuilder`; do the same for `action`/`actionP` so generation-time panics surface as a clean `tui.FatalPanel`. *(added 2026-04-20, source: Plan 15 iter 3 review)*
- **[improvement] `bonsai add` Tech Lead required: duplicate surface** — `cmd/add.go:222` prints a post-harness `tui.ErrorDetail` for the no-tech-lead case, but the splicer at `cmd/add.go:150-153` already showed an in-AltScreen `NewNote` with the same message. After Enter the user sees both. Either suppress the post-harness print when the splicer used the NoteStep branch, or drop the in-harness Note and rely on the durable stdout output. *(added 2026-04-20, source: Plan 15 iter 3 review)*
- **[bug] `runUpdate.applyCustomFileSelection` appends without dedup** — `cmd/update.go:244-285` appends user-picked custom files to the agent's `CustomFiles` list without checking for existing entries. A re-run of `bonsai update` with the same custom file already tracked will append it twice. Pre-existing in the legacy code; carried forward by iter 3. Fix: build a `seen` set from the existing list before appending. *(added 2026-04-20, source: Plan 15 iter 3 review)*
...
- **[bug] `showWriteResults` tree nests cross-workspace files under the new agent's workspace** — When `bonsai add` installs agent X at workspace `A/`, the generator also re-renders files under OTHER agents' workspaces (`SettingsJSON`, `PathScopedRules`, `WorkflowSkills` all iterate `cfg.Agents`). The "Updated" panel then shows those files with A's workspace as the tree root — e.g., adding backend at `src/` displayed `src/sda/.claude/settings.json` even though the actual file is `sda/.claude/settings.json` (tech-lead's workspace, project-rooted). Root cause: `showWriteResults` in `cmd/root.go:131` strips `rootLabel` prefix but the prefix only matches the NEW agent's files; files from other agents keep their full relative path and get rendered under the tree root `rootLabel`. Fix: render with `.` (project root) or split into per-workspace trees. *(added 2026-04-20, source: user dogfooding — Bonsai-Test `bonsai add backend` panel)*
