...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED 2026-05-13 via v0.4.3 hotfix (PR #105/#106). Hooks now bake install-time absolute paths. Existing projects need `bonsai update`. Removed from Backlog by backlog-hygiene 2026-07-27. -->
...
<!-- "[feature] `bonsai init` / `bonsai add` need non-interactive flags" — RESOLVED 2026-05-13 via v0.4.2 release (PR #102). `--non-interactive` + `--from-config` flags shipped for both commands; JSONL stdout + exit codes 0/2/3/4. Removed from Backlog by backlog-hygiene 2026-07-27. -->

_(No active P0 items. See HTML comments above for recently resolved entries.)_

## P1 — High

- **[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove** `[REVIEW: likely resolved by Plan 41]` — Odysseus hub + AI agents must drive Bonsai headless, no TUI. Today: `init`+`add` have `--non-interactive`/`--from-config` (v0.4.2); `update` has NO flags (isatty→`RunStatic` auto-accept, no overlay input); `remove` likely TUI-only. Need a unified non-interactive surface + JSONL/exit-code contract across all four (audit each first). Supersedes Plan 40 Phase 4's update-delivery slice; folds in Rung-2.5 multi-agent `--from-config`. Promote to a plan + grill next session (`/plan`). *(added 2026-06-13, source: user — "main thing"; flagged 2026-07-27 backlog-hygiene — Plan 41 2026-06-16 shipped headless cores + JSONL/exit contract for all 4 cmds, `list --json`, and `docs/agent-interface.md`. Confirm resolution with user before removing.)*

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[OVERDUE — rotate now]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — 90-day window expired ~2026-07-21. **Today is 2026-07-27 — PAT is likely expired and will break the next release brew step.** Rotate via GitHub → Settings → PAT → regenerate, then `gh secret set HOMEBREW_TAP_TOKEN --repo LastStep/Bonsai`. Same applies to any other PATs in repo secrets (audit + document expiry dates). *(added 2026-04-22, source: v0.2.0 release session; flag updated 2026-07-27 backlog-hygiene)*
