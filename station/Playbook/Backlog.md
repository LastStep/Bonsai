...
- **[debt] Node 20 → Node 24 actions migration** — GH workflow runners force Node 24 default 2026-06-02; Node 20 removed 2026-09-16. `actions/checkout@v4` still runs on Node 20 (warning surfaced on every CI + release run). Bump to `@v5` (Node 24) when Dependabot opens PR, or set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` workflow env. Affects ci.yml (3 jobs) + release.yml + codeql.yml + docs.yml. *(added 2026-05-07, source: v0.4.1 release annotations)*
