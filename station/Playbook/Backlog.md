...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED v0.4.3 (PRs #105/#106, 2026-05-13). Absolute install-time paths now baked into .claude/settings.json. Removed from P0 by backlog-hygiene 2026-06-29. -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED v0.4.2 (Plan 39, 2026-05-13). --non-interactive + --from-config shipped for both init and add. Full headless contract (all 4 cmds) shipped Plan 41 (2026-06-16). Removed from P0 by backlog-hygiene 2026-06-29. -->

## P1 — High

<!-- "[feature] Full agent-drivable CLI parity: init / update / add / remove" — RESOLVED Plan 41 (2026-06-16, PRs #120/#122/#123/#121/#125). All 4 cmds have pure *Result headless cores + JSONL/exit contract. MCP server = Plan 42 fast-follow. Removed from P1 by backlog-hygiene 2026-06-29. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** `[ACTION DUE ~2026-07-15 — 16 days]` — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — set calendar reminder for ~2026-07-15 to rotate before next release. Same applies to any other PATs in repo secrets (audit + document expiry dates). Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds (binaries published, only formula update missed). *(added 2026-04-22, source: v0.2.0 release session)*
