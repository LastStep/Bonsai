...
<!-- RESOLVED 2026-07-20 (backlog-hygiene): v0.4.3 hotfix shipped 2026-05-13 — sensor hook commands now bake absolute install-time project root into .claude/settings.json. Users need `bonsai update` to refresh. [bug] Sensor hook commands use $PWD-walk-up — breaks in multi-.bonsai.yaml setups [Plan 39 follow-up] *(added 2026-05-13, source: live session bug)* -->
...
<!-- RESOLVED 2026-07-20 (backlog-hygiene): v0.4.2 shipped 2026-05-13 — `--non-interactive` and `--from-config` flags now exist for init+add (JSONL stdout, exit codes 0/2/3/4). [feature] bonsai init / bonsai add need non-interactive flags [Plan 38 P2 blocker] *(added 2026-05-08, source: Plan 38 P0+P1 dispatch review)* -->

## P1 — High

<!-- RESOLVED 2026-07-20 (backlog-hygiene): Plan 41 — Headless CLI Contract shipped 2026-06-16 (PRs #120/#122/#123/#121/#125). All 4 cmds (init/add/update/remove) now have pure headless cores + JSONL/exit contract (ExitConflict=5); list --json; docs/agent-interface.md contract doc. [feature] Full agent-drivable CLI parity: init/update/add/remove *(added 2026-06-13, source: user)* -->
