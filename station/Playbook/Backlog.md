...
<!-- "[bug] Sensor hook commands use `$PWD`-walk-up" — RESOLVED by v0.4.3 hotfix (PR #105/#106, shipped 2026-05-13). Hook commands now bake install-time absolute paths. Removed from Backlog 2026-08-11 (backlog-hygiene routine). -->
...
<!-- "[feature] `bonsai init` / `bonsai add` need non-interactive flags" — RESOLVED by v0.4.2 release (shipped 2026-05-13). `--non-interactive --from-config <path>` landed in both commands. Removed from Backlog 2026-08-11 (backlog-hygiene routine). -->

_(No active P0 items — all resolved. Sentrux trial is Pending in Status.md.)_

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (PRs #120/#122/#123/#121/#125, shipped 2026-06-16). All 4 mutating commands have headless *Result cores + JSONL/exit contract; `list --json`; agent-interface.md doc. Removed from Backlog 2026-08-11 (backlog-hygiene routine). -->
