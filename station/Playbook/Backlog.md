...
- **[debt] `growSucceeded` predicate in `cmd/add_redesign.go` walks `prev` tail for any error** — `growSucceeded` predicate (lines ~82–94) iterates the full `prev` slice from tail looking for any `error` value to return false. Defensive against Grow publishing an error at an unexpected index, but fragile: if any prior stage (Select/Ground/Graft/Observe) ever starts returning an `error` in its Result payload (or a stage Result leaks an error-typed nil), the Yield conditional silently skips and post-harness cleanup returns. Tighten to explicitly target the Grow slot index (known after splice: `prev[4]` on new-agent happy path), or have Grow publish a sentinel struct the predicate unwraps. Independent review (PR #59) flagged as minor. *(added 2026-04-22, source: PR #59 review)*
- **[improvement] Unknown-agent path in addflow splicer renders `YieldTechLeadRequired` — copy mismatch** — When `cat.GetAgent(agentType)` returns nil in `cmd/add_redesign.go:100–106`, the splicer terminates with `NewYieldTechLeadRequired(ctx, agentType)`. The "tech-lead required" card copy is wrong for that branch — the user picked an unknown agent type, not a known non-tech-lead with tech-lead missing. In practice rare (agent options come from the catalog, so an unknown picked-agent would need a bug upstream), but the copy lies to the user. Add a `NewYieldUnknownAgent` variant with honest "Unknown agent type — catalog may be stale, run `bonsai update`" copy, or collapse the catch to a fatal panel. Independent review (PR #59) flagged as minor. *(added 2026-04-22, source: PR #59 review)*
...
- **[security] `.bak` write-error silent-discard in both conflict-apply helpers** — `cmd/add_redesign.go:281-285` (`applyCinematicConflictPicks`, Plan 23 Phase 2) and `cmd/root.go:158` (legacy `applyConflictPicks`, used by remove/update/legacy add) both `_ = os.WriteFile(abs+".bak", …)` for Backup picks, then proceed into `wr.ForceSelected(toOverwrite, …)` regardless. If the backup write fails (disk full, read-only mount, missing parent), the user ends up with an overwritten file and no backup. Carry-forward bug — not a regression. Remediation: drop the path from `toOverwrite` on backup failure and surface `tui.Warning`. Also: fix the same TOCTOU window where `os.ReadFile(abs)` can fail if the file disappears between conflict detection and picker completion (drop + warn, same pattern). Fix both helpers in one pass. *(added 2026-04-22, source: PR #62 security review)*
- **[debt] `confIdx := len(results) - 2` arithmetic in `cmd/add_redesign.go` post-harness** — Post-harness cleanup resolves the conflicts stage slot via `len(results) - 2` gated on `wr.HasConflicts()`. Works today but a future ConditionalStep added between Conflicts and Yield silently breaks the math. Switch to a type-scan for `map[string]config.ConflictAction` in `results` (same pattern Observe now uses for `GraftResult`). Independent review (PR #62) flagged as minor. *(added 2026-04-22, source: PR #62 review)*
- **[debt] No direct unit test for `applyCinematicConflictPicks`** — Three action branches (Keep=noop, Overwrite=ForceSelected only, Backup=.bak+ForceSelected) + mixed-action inputs are covered only transitively via the splicer. Add a focused test in `cmd/add_redesign_test.go` (or equivalent): synthetic picks map + temp `projectRoot`, assert `.bak` created, `wr.ForceSelected` called with correct list, Keep entries absent from the overwrite set. Independent review (PR #62) flagged as minor. *(added 2026-04-22, source: PR #62 review)*
