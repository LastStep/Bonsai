...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED by v0.4.3 hotfix (PRs #105/#106, 2026-05-13). Absolute paths baked in at install time. Existing projects need `bonsai update`. -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED by v0.4.2 release (2026-05-13). `--non-interactive` + `--from-config` shipped in cmd/init.go and cmd/add.go. -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (PRs #120/#122/#123/#121/#125, 2026-06-16). Every mutating cmd has headless *Result core + JSONL/exit contract; `list --json`; docs/agent-interface.md contract doc. -->
