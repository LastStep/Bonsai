...
- **[feature] Human-AI interaction guide** — Write `docs/working-with-agents.md` — a guide for humans on how to communicate effectively with Bonsai-powered agents. Cover: how to frame requests so the agent picks the right workflow, when to be directive vs exploratory, how memory and session continuity work from the human side, reading and interpreting agent reports, when to intervene vs let the agent iterate, how to give feedback the agent will retain, and how the Tech Lead dispatch model changes the interaction pattern. Position this as "learning the language" — Bonsai creates a structured digital language for AI collaboration, this guide teaches humans to speak it. *(added 2026-04-16, source: user)*
