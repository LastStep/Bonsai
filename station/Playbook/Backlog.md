...
- **[bug] Nondeterministic `OtherAgents` ordering produces phantom file updates** — `AgentWorkspace` at `internal/generate/generate.go:1213` iterates `cfg.Agents` (a `map[string]InstalledAgent`) to build `ctx.OtherAgents`. Go's map iteration order is randomized per process, so `OtherAgents` ends up in different orders across runs. Any template that does `range .OtherAgents` (e.g. `catalog/agents/tech-lead/core/identity.md.tmpl`, `catalog/sensors/scope-guard-files/scope-guard-files.sh.tmpl`, `catalog/sensors/dispatch-guard/dispatch-guard.sh.tmpl`) renders different bytes run-to-run → `writeFile` flags them as `ActionUpdated` even when nothing logically changed → users see mystery files in the "Updated" panel on re-runs of `bonsai add`. Fix: sort `ctx.OtherAgents` by `AgentType` (or workspace) before returning from the builder loop. Same class of bug likely exists wherever we iterate `cfg.Agents` or similar maps for template context. *(added 2026-04-20, source: user dogfooding — Bonsai-Test `bonsai add tech-lead` run-to-run diff)*
