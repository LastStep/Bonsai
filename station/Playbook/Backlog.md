...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED by v0.4.3 hotfix (2026-05-13). Hooks now bake install-time absolute paths; existing projects need `bonsai update` to refresh. Removed from P0 2026-08-17 (backlog-hygiene). -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED by v0.4.2 (2026-05-13). `--non-interactive` and `--from-config` flags shipped for both commands. Removed from P0 2026-08-17 (backlog-hygiene). -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (2026-06-16). All four commands shipped headless cores + JSONL/exit-code contract (ExitConflict=5); `list --json` included. Removed from P1 2026-08-17 (backlog-hygiene). -->
...
- **[feature] Plan 42 — `bonsai mcp` stdio server** — Add `cmd/mcp.go` using `github.com/modelcontextprotocol/go-sdk` (GA). One MCP tool per headless core (init/add/update/remove/list); `readOnlyHint` on read cmds, `destructiveHint` on remove; `outputSchema`+`structuredContent` from `*Result` types (already shaped for this in Plan 41); elicitation for conflict prompts; client-registration docs. Plan 41 was built as the substrate for this; no logic duplication expected. *(added 2026-08-17, source: memory-consolidation routine — Work State referenced as Backlog P2 but not filed)*
