...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — resolved 2026-05-13 v0.4.3 hotfix (PRs #105/#106). Hook commands now bake install-time absolute paths into .claude/settings.json. (removed backlog-hygiene 2026-09-09) -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — resolved 2026-05-13 v0.4.2 release (PR #102). --non-interactive + --from-config now ship on both commands. (removed backlog-hygiene 2026-09-09) -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init/update/add/remove" — resolved 2026-06-16 Plan 41 (PRs #120-125). All 4 cmds have headless *Result cores + JSONL/exit-code contract (ExitConflict=5). See docs/agent-interface.md. (removed backlog-hygiene 2026-09-09) -->

- **[STALE] [ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** — Calendar reminder was set for ~2026-07-15 (90 days from 2026-04-22 rotation). That date has passed (2026-09-09 = 55 days overdue). Verify PAT was rotated — if not, rotate now and update `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai`. *(added 2026-04-22, flagged stale 2026-09-09 — deadline passed)*
