...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — RESOLVED via v0.4.3 hotfix (2026-05-13). Sensor hooks now bake install-time absolute paths. Removed from backlog 2026-09-12 (backlog-hygiene). -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags" — RESOLVED via v0.4.2 (2026-05-13). --non-interactive + --from-config shipped for init+add. Removed from backlog 2026-09-12 (backlog-hygiene). -->

*(P0 section clear — no unresolved critical items.)*

## P1 — High

<!-- "[feature] Full agent-drivable CLI parity" — RESOLVED via Plan 41 (2026-06-16). All four commands (init/add/update/remove) now have headless *Result cores + JSONL/exit contract. MCP server = fast-follow Plan 42 (filed in memory.md). Removed from backlog 2026-09-12 (backlog-hygiene). -->
