...
- **[Plan-27-cosmetic] ConflictsStage render polish bundle** — PR #69 review minors (all non-blocking, low-value). (1) `internal/tui/addflow/conflicts.go:205-210` — `w := s.Width()` with `w<=0 { w=80 }` fallback is dead code; `w` never threaded into `renderBody()`, downstream reads `s.Width()` directly. Either thread `w` through (match `ground.go:View()` pattern) or drop the defaulting. (2) Same file `:300-310` — `listHeight()` doc claims `fixedRows ≈ 14` but code uses `const fixedRows = 15`. Off-by-one drift; comment + const should agree or fixedRows should be derived from body layout. (3) `:310-312` — `fixedRows=15` magic number; extract named const or derive from layout inventory. (4) `renderList` viewport mutation happens inside `View()` not `Update()` — matches existing SelectStage pattern so repo-wide style nit; consider moving to Update in a sweep. (5) `grow.go` calls BOTH `SetRailHidden(true)` AND `SetBodyOnly(true)`; redundant (bodyOnly implies railHidden); drop one. (6) `internal/tui/initflow/generate.go:renderKeyHintsInline` duplicates RenderFooter's join logic; share the joiner helper. *(added 2026-04-22, source: PR #69 review)*
- **[Plan-27-test-gap] Add direct unit tests for PR2 additions** — (1) `SetBodyOnly` on `internal/tui/initflow/generate.go` has no direct test — currently only exercised via addflow's Grow. Direct unit test confirming body-only mode drops chrome would lock the contract. (2) Conflict list viewport scrolling — `TestConflicts_RenderDoesNotPanic` smokes at various dims but no assertion that viewport engages when `len(files) > listH` and follows `s.focus`. Add a 10-conflict/height=20/focus=8 test asserting rendered output contains rows 4..8. (3) `TestConflicts_ColorFollowsAction` asserts non-empty render per action but doesn't verify palette tone DIFFERS across actions; add a Keep-vs-Overwrite rendered-string-differs smoke. (4) `TestConflicts_LowercaseBatchKeysNoOp` tests `o`/`b` lowercase no-op but skips `k`; add `k`-press assertion to lock the contract. *(added 2026-04-22, source: PR #69 review)*
