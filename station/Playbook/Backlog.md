...
- **[bug] Review panel borders break in non-fullscreen terminals** `[Group F]` — During `bonsai init`, the scaffolding Review panel's LipGloss border renders incorrectly when the terminal width is below some threshold (non-maximized window). Likely a fixed-width or min-width issue in `internal/tui/styles.go` panel definition. Need responsive width handling that degrades gracefully below ~100 cols. *(added 2026-04-17, source: session — UI/UX testing)*
...
- **[improvement] Explicit feedback when a required-only section is auto-accepted** — If a category (e.g., Protocols) has only required items, the selection step silently skips it and moves on. Feels jarring — user loses the "I did something" rhythm. Show a brief acknowledgment (e.g., "Protocols: 4 required items auto-selected") before advancing. *(added 2026-04-17, source: session — UI/UX testing)*
- **[improvement] Show counts alongside ability category headers** — The review panel shows categories like "Skills" then lists items. Add counts to the header: "Skills (3)", "Workflows (5)", "Protocols (4 required)". Gives the user a quick at-a-glance read on the shape of their selection. *(added 2026-04-17, source: session — UI/UX testing)*
