...
<!-- "[bug] Sensor hook commands use $PWD-walk-up" — resolved v0.4.3 hotfix (PR #105/#106, commit 584b82b). Hook commands now bake absolute install-time paths. Existing projects need `bonsai update`. Removed from P0 2026-09-04 (backlog-hygiene routine). -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags [Plan 38 P2 blocker]" — resolved v0.4.2 (PR #102, commit 410a5f1). --non-interactive --from-config shipped for init and add. Removed from P0 2026-09-04 (backlog-hygiene routine). -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — resolved Plan 41 (PRs #120/#122/#123/#121/#125, main `ab202c3`, 2026-06-16). All four mutating cmds have pure `*Result` headless cores + JSONL/exit contract (ExitConflict=5); `list --json`. Removed from P1 2026-09-04 (status-hygiene routine). -->
