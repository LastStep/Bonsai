...
<!-- "[bug] Sensor hook commands use $PWD-walk-up — breaks in multi-.bonsai.yaml setups" — FIXED in v0.4.3 (PR #105/#106, 2026-05-13). Hook commands now bake install-time absolute project root; `bonsai update` required on existing projects. Removed from Backlog 2026-08-01 (backlog-hygiene). -->
...
<!-- "[feature] bonsai init / bonsai add need non-interactive flags [Plan 38 P2 blocker]" — FIXED in v0.4.2 (PR #102, 2026-05-13). --non-interactive + --from-config shipped for init and add with JSONL stdout + exit codes. Removed from Backlog 2026-08-01 (backlog-hygiene). -->

## P1 — High

<!-- "[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove" — RESOLVED by Plan 41 (PRs #120/#122/#123/#121/#125, 2026-06-16). All four mutating cmds + list --json now have pure headless cores + JSONL/exit contract (ExitConflict=5). docs/agent-interface.md contract doc shipped. Removed from Backlog 2026-08-01 (backlog-hygiene). -->
