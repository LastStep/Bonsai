...
_(no active P0 items — all resolved as of 2026-09-16 backlog-hygiene run)_

<!-- [bug] Sensor hook commands use $PWD-walk-up → RESOLVED by v0.4.3 (2026-05-13, PRs #105/#106). Hook commands now bake absolute install-time paths. Existing projects need `bonsai update` to refresh. Removed from P0 by backlog-hygiene 2026-09-16. -->
...
<!-- [feature] `bonsai init`/`add` non-interactive flags → RESOLVED by v0.4.2 (2026-05-13, PR #102). `--non-interactive` + `--from-config` shipped for both init and add. Removed from P0 by backlog-hygiene 2026-09-16. -->

## P1 — High

<!-- [feature] Full agent-drivable CLI parity → RESOLVED by Plan 41 (2026-06-16, PRs #120-#125). All four cmds (init/add/update/remove) have pure *Result headless cores + JSONL/exit contract (ExitConflict=5). Removed from P1 by backlog-hygiene 2026-09-16. -->

- **[ops] HOMEBREW_TAP_TOKEN PAT expiry calendar reminder** — Fine-grained PATs default to 90-day expiry. The `HOMEBREW_TAP_TOKEN` secret on `LastStep/Bonsai` was rotated 2026-04-22 — expiry was ~2026-07-15. **[OVERDUE: as of 2026-09-16 the PAT is ~2 months past due — rotate immediately before next release.]** Same applies to any other PATs in repo secrets. Symptom of expired PAT: GoReleaser fails at brew step with `GET https://api.github.com/repos/LastStep/homebrew-tap: 401 Bad credentials` — release otherwise succeeds. *(added 2026-04-22, source: v0.2.0 release session; urgency updated backlog-hygiene 2026-09-16)*
