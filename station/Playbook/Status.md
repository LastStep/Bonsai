...
| BubbleTea foundation + theme system (migrate init/add/remove/update onto shared harness) | 15 | general-purpose | On `ui-ux-testing` branch. Iter 1 + iter 2 + iter 2.1 + iter 3 (full migration) + iter 3.1 (conflict-picker index regression fix) shipped locally. Pending: final audit + merge `ui-ux-testing` → `main` |
