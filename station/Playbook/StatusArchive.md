...
### 2026-06-28 — Status Hygiene resolutions

- **Full agent-drivable CLI parity: init / update / add / remove** `[P1]` — Resolved 2026-06-16 via Plan 41 (PRs #120/#122/#123/#121/#125). All 4 mutating cmds have pure `*Result` headless cores + JSONL/exit contract (`ExitConflict=5`). `list --json` also ships. Contract documented in `docs/agent-interface.md`. MCP server = fast-follow Plan 42.
