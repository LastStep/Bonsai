...
### 2026-09-06 — Status Hygiene routine resolutions

- **[feature] Full agent-drivable (non-interactive) CLI parity: init / update / add / remove** — Resolved by Plan 41 (all 5 phases merged 2026-06-16). Every mutating cmd (init/add/update/remove) now has a pure `*Result` headless core + JSONL/exit contract (`ExitConflict=5`); `list --json`; `docs/agent-interface.md` contract doc. [Plan 41 Status row](../Status.md).
